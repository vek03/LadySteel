...
function Protocol(protocol, status){
    var protocol = document.getElementById(protocol);
    var status = document.getElementById(status);

    if(status.value != 'Protocolo Enviado')
    {
        protocol.value = '';
        protocol.placeholder = 'O B.O Não Será Enviado!';
        protocol.readOnly = true;
        protocol.removeAttribute('required', 'required');
    }
    else
    {
        protocol.placeholder = 'Ex: BO123123123123123';
        protocol.readOnly = false;
